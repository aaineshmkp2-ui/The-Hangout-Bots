  ...
  async execute(interaction) {
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '🛡️ Moderation', value: '`/kick` `/ban` `/timeout` `/warn` `/warnings` `/clear` `/lock` `/unlock` `/jail` `/unjail` `/snipe`' },
        { name: '🎫 Tickets', value: 'Click **Create Ticket** on the panel posted via `/panel` → Tickets.' },
        { name: '🌐 Web Dashboard', value: 'Everything above can also be configured at your dashboard URL — sign in with Discord.' }
      )
